...
//#region node_modules/.nitro/vite/services/ssr/assets/cms-C-fbWjA1.js
/**
...
var SECTION_DEFS = [
	{
		...
		defaults: {
			...
			subtitle: "One message to us, and a checked, honest person helps your parents in India — shopping, repairs, hospital visits, paperwork — with photos and updates sent back to you.",
	...
	},
	{
		...
		fields: [
			...
			{
				key: "groceriesImageUrl",
				label: "Groceries story image URL",
				type: "text"
			},
			{
				key: "repairsImageUrl",
				label: "Home services story image URL",
				type: "text"
			},
			{
				key: "healthcareImageUrl",
				label: "Healthcare story image URL",
				type: "text"
			},
			{
				key: "items",
		...
		defaults: {
			...
			subtitle: "Instead of searching for many different people, just ask SAFAR N MANZIL. Here is how we handle your requests — and we tell you the price before we start.",
			groceriesImageUrl: "",
			repairsImageUrl: "",
			healthcareImageUrl: "",
			items: [
				{
					...
					description: "Surprise them with gifts or make sure their weekly groceries arrive without them lifting a finger. You see the bill."
				},
				{
					title: "Parcel & logistics",
					description: "We pick up papers and parcels, send them and tell you where they are."
				},
				{
					title: "Trusted home repairs",
					description: "From plumbing to electrical work, we send verified, honest professionals so your family is never taken advantage of. Price fixed first."
				},
				{
					title: "Medical & errands",
					description: "We arrange safe transport and go with your parents to hospital visits, bring medicines and run their local errands — then tell you how it went."
				},
				{
					title: "Documentation & legal",
					description: "Certificates, bank and government work. We keep going until the paper is in hand."
				},
				{
					title: "Property & coordination",
					description: "We visit your house or land, help with rent, check repairs and send photos."
				}
			]
		}
	},
	...
	},
	{
		...
		defaults: {
			...
			subtitle: "When the work is finished, our person sends a photo, the bill and a short message. You see it on your phone, wherever you are.",
			imageUrl: "",
			points: [
				"A photo of the work",
				"The shop or worker bill",
				"Our fee shown separately",
			...
			items: [
				{
					...
					description: "We check the ID of every worker we send, and we rate them after each job."
				},
				{
					title: "Price told before work",
					description: "You say yes to the cost first. We show the shop or worker cost and our fee separately."
				},
				{
					title: "Proof for every job",
					description: "Photos, bills and updates for every job, saved in one place you can check any time."
				}
			]
		}
	},
			...
			steps: [
				{
					...
					description: "Send us a message on WhatsApp or fill the form on this page."
				},
				{
					title: "We tell you the cost",
					description: "We tell you the plan, who will do it and the price before we start."
				},
				{
					title: "We get the work done",
					description: "Our person does the work while we keep an eye on it."
				},
				{
					title: "You get photos and a bill",
					description: "We send photos, bills and a simple invoice when it is finished."
				}
			]
		}
	},
	{
		...
		defaults: {
			...
			items: [{
				quote: "The guilt of not being there for my parents was killing me. Now, if the AC breaks or they need groceries, I just message SAFAR N MANZIL from Dubai. They handle my parents like their own family. Priceless.",
		...
		defaults: {
			...
			items: [
				...
				},
				{
					...
					answer: "You agree the cost first. Then you get a bill that shows the shop or worker cost and our fee separately."
				},
				{
...
var SECTION_MAP = new Map(SECTION_DEFS.map((def) => [def.key, def]));
function defaultContent(key) {
	return structuredClone(SECTION_MAP.get(key)?.defaults ?? {});
}
...
async function fetchPublishedContent() {
	...
	const { data, error } = await supabase.from("cms_sections").select("section_key, content, status").eq("page", "home").eq("status", "published");
	if (error) return merged;
	for (const row of data ?? []) {
		const key = row.section_key;
		const content = row.content;
		if (SECTION_MAP.has(key) && content && Object.keys(content).length > 0) merged[key] = {
			...merged[key],
			...content
		};
	}
...
async function fetchSectionRows() {
	const supabase = getSupabase();
	if (!supabase) return [];
	const { data, error } = await supabase.from("cms_sections").select("id, page, section_key, content, draft_content, status, version, updated_at").eq("page", "home");
	if (error) throw new Error(error.message);
	return data ?? [];
}
async function saveDraft(sectionKey, content) {
...
async function publishSection(sectionKey, content) {
	const supabase = client();
	const { data: existing } = await supabase.from("cms_sections").select("id, version").eq("page", "home").eq("section_key", sectionKey).maybeSingle();
	const version = (existing?.version ?? 0) + 1;
	const { data: saved, error } = await supabase.from("cms_sections").upsert({
		page: "home",
		section_key: sectionKey,
		content,
		draft_content: content,
		...
		version,
		updated_at: (/* @__PURE__ */ new Date()).toISOString()
	}, { onConflict: "page,section_key" }).select("id").single();
	if (error) throw new Error(error.message);
	const sectionId = saved.id;
	const { error: revisionError } = await supabase.from("cms_revisions").insert({
...
async function unpublishSection(sectionKey) {
	const { error } = await client().from("cms_sections").update({
		status: "draft",
		updated_at: (/* @__PURE__ */ new Date()).toISOString()
	}).eq("page", "home").eq("section_key", sectionKey);
	if (error) throw new Error(error.message);
}
async function fetchRevisions(sectionId) {
	const supabase = getSupabase();
	if (!supabase) return [];
	const { data, error } = await supabase.from("cms_revisions").select("id, section_id, content, version, saved_at").eq("section_id", sectionId).order("version", { ascending: false }).limit(20);
	if (error) throw new Error(error.message);
	return data ?? [];
}
//#endregion
export { fetchRevisions as a, saveDraft as c, fetchPublishedContent as i, unpublishSection as l, defaultContent as n, fetchSectionRows as o, defaultSections as r, publishSection as s, SECTION_DEFS as t };
